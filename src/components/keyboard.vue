<template>
  <div class='kb-content' :class='!keyboardShow && "kb-hide"'>
    <div class='kb-left'>
      <div v-for="(item, index) in keyArr" class='kb-item kb-left-item b-r' :key="index" :class="index < 9 && 'b-b'" :style="{height: itemH + 'px', lineHeight: itemH + 'px'}" @click="handleInput(item)">{{item}}</div>
      <div class='kb-item kb-left-item b-r' :style="{height: itemH + 'px', lineHeight: itemH + 'px'}" @click='keyboardToggle(false)'>
        <img class='submit-icon' :src="require('../assets/images/hide.png')" />
      </div>
    </div>
    <div class='kb-right'>
      <div @click='handleDelete' class='kb-item kb-right-item k-cancel b-b' :style="{height: itemH * 2 + 'px', lineHeight: itemH * 2 + 'px'}">
        <img class='delete-icon' :src="require('../assets/images/delete.png')" />
      </div>
